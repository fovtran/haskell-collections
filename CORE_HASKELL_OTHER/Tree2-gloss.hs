...
main = animate (InWindow "Clock" (600, 600) (20, 20)) black frame
...
clockFractal n s  = Pictures [circ1, circ2, circ3, lines]
 where
  ...
  --  the animation started.
    ...
    circNm1 = Pictures
      [ circle 1, Scale (a/2.5) (a/2.5) $ clockFractal (n-1) s
      , if n > 2 then Color cyan $ Translate (-0.15) 1 $ Scale 0.001 0.001 $ Text (show s) 
       else Blank  ]
